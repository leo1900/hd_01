...
namespace DragonU3DSDK.Asset
{
    public static partial class BuildAssetBundles
    {
        ...
        [MenuItem("AssetBundle/根据AssetConfigPrototype更新AssetConfigCtrl")]
        public static void RefreshAssetConfigControllerByAssetConfigPrototype()
        {
            ...
            for (int i = 0; i < AssetConfigController.Instance.Groups.Length; i++)
            {
                ...
                if (paths != null && paths.Count > 0)
                {
                    foreach (var p in paths)
                    {
                        ...
                        {
                            throw new Exception($"需要回滚数据:{p}，但是备份数据不完整。");
                        }
        ...
        static void AfterBuildAllAssetBundleCheck()
        {
            ...
            if (!checkLocalJson.Equals(checkCalculateJson))
            {
                ...
                throw new Exception("two version not equal int compare with calculate!!!");
            }
